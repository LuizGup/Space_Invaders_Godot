...
@onready var label = $MarginContainer/CenterContainer/GameOverContainer/GameOverLabel
@onready var restart_button = $MarginContainer/CenterContainer/GameOverContainer/GameOverButton

@export var invader_spawner: InvaderSpawner
@export var life_manager: LifeManager

func _ready():
	points_label.text = "PONTOS: %d" % 0
...
func points_increased(points: int):
	points_label.text = "PONTOS: %d" % points
